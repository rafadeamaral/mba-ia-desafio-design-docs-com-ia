> **Nota:** este é o enunciado original do desafio, preservado sem alterações. Ele ocupava o `README.md` da raiz do repositório base, que foi substituído pela documentação do processo de produção (requisito 6). A entrega está em [`PRD.md`](PRD.md), [`RFC.md`](RFC.md), [`FDD.md`](FDD.md), [`adrs/`](adrs/) e [`TRACKER.md`](TRACKER.md).
